    ...
    <Card className="w-full max-w-[450px]">
      <CardHeader className="justify-between">
        <div className="flex gap-5">
          ...
          <div className="flex flex-col gap-1 items-start justify-center">
            <h4 className="text-small font-semibold leading-none text-default-600">{tweet.user.name}</h4>
            <h5 className="text-small tracking-tight text-default-400">@{tweet.user.username}</h5>
          </div>
        </div>
      </CardHeader>
      ...
      <CardFooter className="gap-6">
        ...
        <button
          ...
          className={`${retweeted ? "text-success" : "text-default-400"} transition flex gap-1 items-center justify-center hover:text-success cursor-pointer`}>
          <p className="font-semibold text-small">{tweet.retweetedBy.length}</p>
          <FaRetweet className="text-lg" />
        </button>
        <button
          onClick={handleLike}
          className={`${liked ? "text-danger" : "text-default-400"} transition flex gap-1 items-center justify-center text-small hover:text-danger cursor-pointer`}>
